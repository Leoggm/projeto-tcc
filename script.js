...
function showRecommendation(doenca) {
  ...
  document.getElementById('feedbackForm').addEventListener('submit', function(event) {
    ...
    dadosFeedback.append('entry.1269783042', nome);
    dadosFeedback.append('entry.2060248135', email);
    dadosFeedback.append('entry.1812151574', feedback);

    // Envia os dados para o Google Forms usando fetch
    fetch('https://docs.google.com/forms/d/e/1FAIpQLSewi6p0Cn68kmbuOK-1AuSuaEvBcR6rbovCYjs76o8yPZkpjQ/viewform?usp=sf_link', {
        method: 'POST',
        body: dadosFeedback
    })
    .then(response => {
        if (response.ok) {
            ...
            showErrorMessage('Ocorreu um erro ao enviar o formulário. Por favor, tente novamente.');
        }
    })
    .catch(() => {
        showErrorMessage('Ocorreu um erro ao enviar o formulário. Por favor, tente novamente.');
